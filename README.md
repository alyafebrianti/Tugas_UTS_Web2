...
```
Saat payload ' OR '1'='1' --  dimasukkan, query berubah menjadi:

SQL
```sql
SELECT id, username, role FROM users WHERE username = '' OR '1'='1' -- ' AND password = '...' LIMIT 1
Logika '1'='1' selalu bernilai TRUE, dan tanda --  mematikan pengecekan password di belakangnya.
```
Solusi Mitigasi
Untuk mencegah serangan ini, gunakan Prepared Statements (Parameterization) yang memisahkan instruksi SQL dari data pengguna:
...
## Kesimpulan 
Melalui eksperimen ini, dapat disimpulkan bahwa Keamanan Bukan Opsional Celah SQL Injection adalah bukti nyata bahwa mengandalkan input pengguna tanpa filter sangatlah berbahaya. 
